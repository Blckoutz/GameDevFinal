...
func _ready() -> void:
	...
	$FLightBox/FLightBox.disabled = true  # Make sure initial state is off
	$FLightBox/FLightBox/flashlight.enabled = false  # Make sure initial state is off

func _physics_process(delta: float) -> void:
	player_movement(delta)
	flashlight_on()
	flashlight_battery(delta)
	GameGlobals.player_position = global_position  # Update the global player position
...
func flashlight_on() -> void:
	if Input.is_action_just_pressed("flashlight"):
		...
		$FLightBox/FLightBox.disabled = not FLon  # Enable/disable based on state
		$FLightBox/FLightBox/flashlight.enabled = FLon  # Enable/disable the actual light

func flashlight_battery(delta: float) -> void:
	if FLon:
		battery_life -= delta  # Decrease battery life based on time passed
		if battery_life <= 0:
			FLon = false
